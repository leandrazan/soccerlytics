...
draw_pitch <- function (dimensions = c(120, 80), palette = "gw", color = NULL, 
  ...
  arg_list <- list(boundary = list(geom = "rect", xmin = x_min, 
                   ...
                   left_box_arc = list(geom = "curve",  x = 16.5 * x_adj, xend = 16.5 * x_adj,
                                       y = y_mid - (ifelse(coord_flip, -1, 1) * sqrt((y_adj * 5.5)^2 + (y_adj * 9.15)^2)), 
                                       yend = y_mid + (ifelse(coord_flip,-1, 1) * sqrt((y_adj * 5.5)^2 + (y_adj * 9.15)^2)), 
                                       color = palette_color), 
                   right_box_arc = list(geom = "curve",  x = x_max - (16.5 * x_adj), xend = x_max - (16.5 * x_adj), y = y_mid + (ifelse(coord_flip, -1, 1) * sqrt((y_adj * 5.5)^2 + (y_adj * 9.15)^2)), 
                                        yend = y_mid - (ifelse(coord_flip, -1, 1) * sqrt((y_adj * 5.5)^2 +  (y_adj * 9.15)^2)), 
                                        color = palette_color), 
